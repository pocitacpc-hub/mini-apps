/* Převody – app.js (umístění: apps/prevody/app.js)
 * Čte data z ./data.json (tedy apps/prevody/data.json)
 * Vyžaduje v HTML elementy: #configGrid, #compareChart, #chartLegend
 * Export XLSX přes SheetJS (CDN v HTML).
 */
...
async function loadData() {
  ...
  if (!res.ok) throw new Error("Nepodařilo se načíst data.json. Spusť přes lokální server.");
  return res.json();
}
...
function buildConfigCard(id) {
    ...
    el("div", { class: "head-actions" }, [
      el("label", { class: "toggle" }, [
        el("input", {
          type: "checkbox",
          ...(cfg.enabled ? { checked: "" } : {}),
          onChange: () => { cfg.enabled = !cfg.enabled; render(); }
        }),
        el("span", {}, [ "Aktivní" ])
      ]),
      el("button", { class: "icon-btn", onClick: () => duplicateConfig(id) }, ["Duplikovat"]),
      el("button", { class: "icon-btn", onClick: () => resetConfig(id) }, ["Reset"])
    ])
  ]);
  ...
  return el("div", { class: "config-card" }, [head, body]);
}
...
function buildConfigForm(id) {
  ...
  const pairOpts = DATA.chainring_pairs_2x.map(p =>
    el("option", { value: p.id, ...(p.small === cfg.ring2xSmall && p.big === cfg.ring2xBig ? { selected: "" } : {}) }, [p.label])
  );
  ...
  const form = el("div", { class: "form" }, [
    el("div", { class: "field half" }, [
      el("label", {}, [ "Název" ]),
      el("input", { type:"text", value: cfg.name, onInput: (e)=>{ cfg.name = e.target.value; render(); } })
    ]),
    el("div", { class: "field half" }, [
      el("label", {}, [ "Pohon" ]),
      el("div", { class: "inline" }, [
        el("label", { class: "radio" }, [
          el("input", { type:"radio", name:`drive-${id}`, ...(cfg.drivetrain==="1x"?{checked:""}:{}),
            onChange: ()=>{ cfg.drivetrain="1x"; render(); }
          }),
          el("span", {}, ["1x"])
        ]),
        el("label", { class: "radio" }, [
          el("input", { type:"radio", name:`drive-${id}`, ...(cfg.drivetrain==="2x"?{checked:""}:{}),
            onChange: ()=>{ cfg.drivetrain="2x"; render(); }
          }),
          el("span", {}, ["2x"])
        ])
      ])
    ...
    el("div", { class: "field half" }, [
      el("label", {}, [ "Kolo" ]),
      el("select", {
        onChange: (e)=>{ cfg.wheelId = e.target.value; const w=byId(DATA.wheels,cfg.wheelId); if(w) cfg.customCircMm = w.circumference_mm; render(); }
      }, wheelOpts)
    ]),
    el("div", { class: "field half" }, [
      el("label", {}, [ "Obvod (mm)" ]),
      el("div", { class: "inline" }, [
        el("label", { class: "toggle" }, [
          el("input", { type:"checkbox", ...(cfg.useCustomCirc?{checked:""}:{}),
            onChange: ()=>{ cfg.useCustomCirc = !cfg.useCustomCirc; render(); }
          }),
          el("span", {}, [ "Vlastní obvod" ])
        ]),
        el("input", {
          type:"number", min:"1000", max:"4000", step:"1",
          value: cfg.customCircMm,
          disabled: cfg.useCustomCirc ? null : "",
          onInput: (e)=>{ cfg.customCircMm = Number(e.target.value || cfg.customCircMm); render(); }
        })
      ])
    ]),
    ...
    el("div", { class: "field half" }, [
      el("label", {}, [ "Kazeta" ]),
      el("select", { onChange: (e)=>{ cfg.cassetteId = e.target.value; render(); } }, cassetteOpts)
    ]),
    el("div", { class: "field half" }, [
      el("label", {}, [ "Křížení řetězu" ]),
      el("select", { onChange: (e)=>{ cfg.crossMode = e.target.value; render(); } }, crossModeOpts)
    ]),
    ...
    ...(cfg.drivetrain === "1x"
      ? [
          el("div", { class:"field half" }, [
            ...
            el("select", { onChange: (e)=>{ cfg.ring1x = Number(e.target.value); render(); } }, ring1xOpts)
          ])
        ]
      : [
          el("div", { class:"field half" }, [
            el("label", {}, [ "Předvolba 2x" ]),
            el("select", {
              onChange: (e)=>{
                const p = DATA.chainring_pairs_2x.find(x=>x.id===e.target.value);
                if (p) { cfg.ring2xSmall=p.small; cfg.ring2xBig=p.big; }
                render();
              }
            }, [ el("option", { value:"custom" }, ["(vlastní)"]), ...pairOpts ])
          ]),
          el("div", { class:"field half" }, [
            el("label", {}, [ "Převodníky (2x)" ]),
            el("div", { class:"inline" }, [
              el("input", { type:"number", min:"20", max:"70", step:"1", value: cfg.ring2xSmall,
                onInput: (e)=>{ cfg.ring2xSmall = Number(e.target.value||cfg.ring2xSmall); if(cfg.ring2xBig<=cfg.ring2xSmall) cfg.ring2xBig=cfg.ring2xSmall+1; render(); }
              }),
              el("span", { class:"mini" }, ["malý"]),
              el("input", { type:"number", min:"20", max:"70", step:"1", value: cfg.ring2xBig,
                onInput: (e)=>{ cfg.ring2xBig = Number(e.target.value||cfg.ring2xBig); if(cfg.ring2xBig<=cfg.ring2xSmall) cfg.ring2xSmall=Math.max(20,cfg.ring2xBig-1); render(); }
              }),
    ...
    el("div", { class:"field third" }, [
      el("label", {}, ["Kadence (rpm) – per konfigurace"]),
      el("input", {
        type:"number", min:"1", max:"250", step:"1",
        value: cfg.cadence,
        disabled: state.cadencePerConfig ? null : "",
        onInput: (e)=>{ cfg.cadence = Number(e.target.value || 80); render(); }
      })
    ]),

    ...(cfg.crossMode === "advanced"
      ? [
          el("div", { class:"field" }, [
            el("label", {}, [ "Advanced pravidla křížení (N)" ]),
            el("div", { class:"inline" }, [
              el("span", { class:"mini" }, [ "2x: big ring + největší N:" ]),
              el("input", { type:"number", min:"0", max:"6", step:"1", value: cfg.adv_2x_bigRing_largestCogsBad,
                onInput: (e)=>{ cfg.adv_2x_bigRing_largestCogsBad = Number(e.target.value||0); render(); }
              }),
              el("span", { class:"mini" }, [ "small ring + nejmenší N:" ]),
              el("input", { type:"number", min:"0", max:"6", step:"1", value: cfg.adv_2x_smallRing_smallestCogsBad,
                onInput: (e)=>{ cfg.adv_2x_smallRing_smallestCogsBad = Number(e.target.value||0); render(); }
              }),
              el("span", { class:"mini" }, [ "1x: nejmenší N:" ]),
              el("input", { type:"number", min:"0", max:"6", step:"1", value: cfg.adv_1x_smallestCogsBad,
                onInput: (e)=>{ cfg.adv_1x_smallestCogsBad = Number(e.target.value||0); render(); }
              }),
              el("span", { class:"mini" }, [ "největší N:" ]),
              el("input", { type:"number", min:"0", max:"6", step:"1", value: cfg.adv_1x_largestCogsBad,
                onInput: (e)=>{ cfg.adv_1x_largestCogsBad = Number(e.target.value||0); render(); }
              })
            ])
          ])
        ]
...
function buildSummary(id, calc) {
  ...
  box.innerHTML = "";
  items.forEach(it => box.appendChild(
    el("div", { class:"kpi" }, [
      el("div", { class:"k" }, [it.k]),
      el("div", { class:"v" }, [it.v])
    ])
...
function renderConfigs() {
  const grid = document.getElementById("configGrid");
  grid.innerHTML = "";
  for (const id of CFG_IDS) grid.appendChild(buildConfigCard(id));
...
function drawCompareChart() {
  ...
  document.getElementById("chartLegend").innerHTML = ids.map(id => {
...
function resetConfig(id) {
  const wasEnabled = state.configs[id].enabled;
...
function wireTopControls() {
  document.querySelectorAll(".chip").forEach(btn => {
    btn.addEventListener("click", () => {
      const v = Number(btn.dataset.cad);
      state.cadenceGlobal = v;
      document.getElementById("cadenceGlobal").value = v;
      render();
    });
  });

  document.getElementById("cadenceGlobal").addEventListener("input", (e) => {
    state.cadenceGlobal = Number(e.target.value || 80);
    render();
  });

  document.getElementById("cadencePerConfig").addEventListener("change", (e) => {
    state.cadencePerConfig = e.target.checked;
    render();
  });

  document.querySelectorAll(".seg-btn[data-graph]").forEach(b => b.addEventListener("click", () => {
  ...
  document.getElementById("showSpeed").addEventListener("change", e => { state.show.speed = e.target.checked; render(); });
  document.getElementById("showRatio").addEventListener("change", e => { state.show.ratio = e.target.checked; render(); });
  document.getElementById("showDev").addEventListener("change", e => { state.show.dev = e.target.checked; render(); });
  document.getElementById("showGI").addEventListener("change", e => { state.show.gi = e.target.checked; render(); });

  document.getElementById("filterOnlyOk").addEventListener("change", e => { state.filterOnlyOk = e.target.checked; drawCompareChart(); });

  document.getElementById("exportXlsx").addEventListener("click", exportXlsx);

  window.addEventListener("resize", drawCompareChart);
}
...
(async function main(){
  // nespouštěj, pokud jsme omylem na jiné stránce
